...
export function InventarioMetricsCards({ metrics }: InventarioMetricsCardsProps) {
  const cards = [
    ...
    },
    {
      ...
      value: formatEuro(metrics.lucroRealizado),
          ...
          {(counts[f.key] ?? 0) > 0 && (
            <span
              className={`ml-1.5 rounded-full px-1.5 py-0.5 text-xs ${
                filtroAtivo === f.key ? 'bg-white/20' : 'bg-slate-100'
              }`}
            >
              {counts[f.key]}
            </span>
...
function ArtigoFoto({ fotoUrl, nome }: { fotoUrl: string | null; nome: string }) {
  if (fotoUrl) {
    return (
          ...
          >
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-[11px] text-slate-400">#{artigo.id_artigo}</p>
              </div>
            </div>
            ...
            {artigo.url_vinted && (
              <a
                ...
                className="mt-3 block rounded-lg border border-sky-200 bg-sky-50 py-2 text-center text-xs font-medium text-sky-700 hover:bg-sky-100"
              >
                Ver na Vinted ↗
              </a>
            )}
